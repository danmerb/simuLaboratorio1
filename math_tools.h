...
void MatrixInversa(Matrix M, Matrix &Manswer){  
  Matrix transp_matrix, cofac_matrix;
  float d = determinant(M);
  if(d==0){
    exit(EXIT_FAILURE);
  }
  else {
  cofactors(M, cofac_matrix);  
  transpose(cofac_matrix,transp_matrix); 
  productRealMatrix(1/d,transp_matrix,Manswer);
  }
}